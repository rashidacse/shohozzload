...
class Report extends Role_Controller {
    ...
    /*
     * this method will show total report page
     * @author nazmul hasan on 20th october 2016
     */
    public function get_total_report() {
        ...
        if (file_get_contents("php://input") != null) {
            ...
            $profit_list_array = $this->cost_profit_library->get_profit_history($status_id_list, $service_id_list, $from_date, $to_date, $limit, $offset, $where);
        ...
        $profit_list = array();
        $profit_list_array = $this->cost_profit_library->get_profit_history(array(), array(), $current_date, $current_date, $limit, $offset, $where);
        if (!empty($profit_list_array)) {
        ...
        if (!empty($service_list_array)) {
            foreach ($service_list_array as $service_info) {
                $temp_service_info = array(
                    'id' => $service_info['service_id'],
                    'title' => $service_info['title'],
                );
                $service_list[] = $temp_service_info;
            }
        }
    ...
    /*
     * this method will return detail report of a user
    ...
    public function get_detailed_report() {
        ...
        if (file_get_contents("php://input") != null) {
            ...
            $report_information = $this->cost_profit_library->get_report_detail_history($from_date, $to_date, $user_id_list);
        ...
        $report_information = $this->cost_profit_library->get_report_detail_history($current_date, $current_date, $user_id_list);
        if (!empty($report_information)) {
    ...
    public function get_cost_and_profit() {
        $user_id = $this->session->userdata('user_id');
        $this->load->library('Cost_profit_library');
        $user_profits = $this->cost_profit_library->get_user_service_profits($user_id);
    ...
    public function get_user_profit_loss() {
        ...
        $offset = TRANSACTION_PAGE_DEFAULT_OFFSET;
        if (file_get_contents("php://input") != null) {
            $response['report_list'] = array();
            $response['report_summary'] = array();
            $postdata = file_get_contents("php://input");
            $requestInfo = json_decode($postdata);
            if (property_exists($requestInfo, "searchInfo") != FALSE) {
                $search_param = $requestInfo->searchInfo;
