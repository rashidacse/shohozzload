...
//                send verification code via email to the client
    ...
    public function bkash($transaction_id = '') {
        ...
        if (file_get_contents("php://input") != null) {
            //if both cash in and cash out services are switched off after view page is loadded by the user
            ...
            if (property_exists($requestInfo, "bkashInfo") != FALSE) {
                ...
                //checking whethe service id is set and if it is set it must be either cash in or cash out
                if (property_exists($bkashInfo, "bkashServiceId") &&($bkashInfo->bkashServiceId === SERVICE_TYPE_ID_BKASH_CASHIN || $bkashInfo->bkashServiceId === SERVICE_TYPE_ID_BKASH_CASHOUT)) {
                    $service_id = $bkashInfo->bkashServiceId;
                } else {
                    $response["message"] = "Please select either Cash In or Cash Out..";
                    echo json_encode($response);
                    return;
                }
                //checking whether the selected service is restricted or not
                if ($service_info_list[$service_id]['type_id'] == SERVICE_TYPE_ID_NOT_ALLOW_TRNASCATION) {
                    $response["message"] = "Sorry !! The service is unavailable right now! please try again later!.";
                    echo json_encode($response);
                    return;
                }
                if(!in_array($service_id, $bkash_assigned_service_id_list))
                {
                    $response["message"] = "Sorry !! You have no permission to use this service. Please contact with system admin.";
                    echo json_encode($response);
                    return;
                ...
                //code feature will be updated later
                /*if (property_exists($bkashInfo, "code")) {
                    $code = $bkashInfo->code;
                    //verify code here
                    if ($bkash_service_info['sms_verification'] == 1 || $bkash_service_info['email_verification'] == 1) {
            ...
            {
                $response["message"] = "Invalid transaction info. Please try again later.";
                echo json_encode($response);
                return;
            }
            ...
            {
                $api_key = API_KEY_BKASH_CASHOUT;
                $type_id = TRANSACTION_TYPE_ID_CASHOUT;
            }
            else
            {
                $response["message"] = "Invalid API Key for bkash service.";
                echo json_encode($response);
                return;
            }
            ...
            if (!empty($pin_info_array)) {
                $pin = $pin_info_array[0]['pin'];
                if($code != $pin)
                {
                    $response["message"] = "Invalid pin. Please try again later.";
                    echo json_encode($response);
                    return;
                }
            } else {
            ...
            if ($transaction_id == '') {
                ...
                $response['message'] = "To update transaction, please contact with system admin.";
        ...
        if (!$permission_exists) {
            $this->data['app'] = TRANSCATION_APP;
            $this->data['error_message'] = "Sorry !! You have no permission to use this service. Please contact with system admin.";
            $this->template->load(null, 'common/error_message', $this->data);
            return;
        }
        ...
        //verification feature with be modified later. So it is commented now.
        ...
        $this->data['user_service_info_list'] = json_encode($user_service_info_list);
        
        $this->data['transaction_info'] = json_encode($transaction_info);
        $this->data['bkash_service_list'] = $this->utils->get_bkash_service_list($bkash_assigned_service_id_list);
        $transaction_list_array = $this->transaction_library->get_user_transaction_list(array($user_id), array(SERVICE_TYPE_ID_BKASH_CASHIN, SERVICE_TYPE_ID_BKASH_CASHOUT), array(), '', 0, 0, TRANSACTION_PAGE_DEFAULT_LIMIT, 0);
    ...
    public function dbbl() {
        ...
        if (file_get_contents("php://input") != null) {
            ...
            if (isset($amount)) {
                if ($amount < (int)DBBL_MINIMUM_CASH_IN_AMOUNT) {
                    $response["message"] = "Please give a minimum amount TK. " . DBBL_MINIMUM_CASH_IN_AMOUNT . "!";
                    echo json_encode($response);
                    return;
                }
                if ($amount > (int)DBBL_MAXIMUM_CASH_IN_AMOUNT) {
    ...
    public function mcash() {
        ...
        if (file_get_contents("php://input") != null) {
            ...
            if (isset($amount)) {
                if ($amount < (int)MCASH_MINIMUM_CASH_IN_AMOUNT) {
                    $response["message"] = "Please give a minimum amount TK. " . MCASH_MINIMUM_CASH_IN_AMOUNT . "!";
                    echo json_encode($response);
                    return;
                }
                if ($amount > (int)MCASH_MAXIMUM_CASH_IN_AMOUNT) {
    ...
    public function ucash() {
        ...
        if (file_get_contents("php://input") != null) {
            ...
            if (isset($amount)) {
                if ($amount < (int)UCASH_MINIMUM_CASH_IN_AMOUNT) {
                    $response["message"] = "Please give a minimum amount TK. " . UCASH_MINIMUM_CASH_IN_AMOUNT . "!";
                    echo json_encode($response);
                    return;
                }
                if ($amount > (int)UCASH_MAXIMUM_CASH_IN_AMOUNT) {
    ...
    public function topup() {
            ...
            if (property_exists($requestInfo, "transactionDataList")) {
                ...
                foreach ($transaction_data_list as $key => $transaction_data) {
                    ...
                    if (property_exists($transaction_data, "amount")) {
                        ...
                        if(array_key_exists($topup_data_info['service_id'], $service_id_user_service_info_map) && $service_id_user_service_info_map[$topup_data_info['service_id']]['rate'] != 0 )
                        {
                            $topup_data_info['cost'] = (double)((double)$amount + (double)$amount / $service_id_user_service_info_map[$topup_data_info['service_id']]['rate'] * $service_id_user_service_info_map[$topup_data_info['service_id']]['charge']);
                        }
                        else
                        {
                            $response["message"] = "Invalid charge configuration for this service at serial " . ($key + 1);
                            echo json_encode($response);
                            return;
                        }
                        $total_cost = (double)((double)$total_cost + (double)$topup_data_info['cost']);
                    ...
                    if (property_exists($transaction_data, "code")) 
                    {
                ...
                if ($this->transaction_library->add_multipule_transactions($transction_list, $user_assigned_service_id_list, $total_cost, $user_id) !== FALSE) {
                    $response['message'] = $this->transaction_library->messages_array();
                } else {
                    $response['message'] = $this->transaction_library->errors_array();
                }
        ...
        $this->data['code_verification'] = true;
        ...
        $this->data['service_id_user_service_info_map'] = json_encode($service_id_user_service_info_map);
        $this->data['transaction_list'] = json_encode($transaction_list);
        $this->load->model('service_model');
        $topup_type_list = $this->utils->get_topup_types();
