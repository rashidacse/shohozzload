...
class Transaction_library {
    ...
    public function add_transaction($api_key, $transaction_data) {
        ...
        $response = $this->payment_library->is_transaction_amount_permitted($user_id, $amount);
        if (!$response['status']) {
     ...
     * @param $total_cost, total required cost for transactions
     * @param $user_id, user_id
     * @author rashida on 9th march 2016
     */

    public function add_multipule_transactions($transaction_list, $service_id_list, $total_cost, $user_id = 0) {
        if ($user_id == 0) {
            $user_id = $this->session->userdata('user_id');
        }
        //checking whether user has enough balance before the transaction
        $this->load->library('payment_library');
        $response = $this->payment_library->is_transaction_amount_permitted($user_id, $total_cost, TRANSACTION_BALANCE_CHECK_USER_BALANCE);
        if (!$response['status']) {
            $this->set_error('error_insufficient_balance');
            return FALSE;
        }
...
//        }
        ...
        return $this->transaction_model->add_transactions($transaction_list, $users_profit_list, $response['balance']);
    }
...
//    }
    ...
    public function calculate_multipule_transactions_profit_chain($user_id, $service_id_list, $transaction_list) {
        ...
        foreach ($user_service_list as $service_info) {
            $key = $service_info['user_id']."_".$service_info['service_id'];
            $user_id_service_info_map[$key] = $service_info;
        }
        $user_profit_list = array();
        foreach ($user_service_list as $service_info) {
            foreach ($transaction_list as $transction_info) {
                if ($transction_info['service_id'] == $service_info['service_id']) {
                    ...
                    if($service_info['rate'] != 0)
                    {
                        $commission = 0;
                        //$charge = 0;
                        if ($service_info['user_id'] == $user_id) {
                            $commission = (double)$amount / $service_info['rate'] * $service_info['commission'];
                        } else {                            
                            $commission = (double)((double)$amount / $service_info['rate'] * ($service_info['commission'] - (double)$user_id_service_info_map[$parent_child_id_map[$service_info['user_id']]."_".$service_info['service_id']]['commission']));
                            //$charge = (double)((double)$amount / $service_info['rate'] * ($service_info['charge'] - (double)$user_id_service_info_map[$parent_child_id_map[$service_info['user_id']]."_".$service_info['service_id']]['charge']));
                        }
                        
                        //$total_commission = (double)($commission+$charge);
                        //we are adding commission info at profit table if we have a valid commission
                        if($commission > 0)
                        {
                            $user_profit_info['commission'] = $commission;
                            $user_profit_list[] = $user_profit_info;
                        }                        
                    }
    ...
    public function get_pending_request_statuses() {
        ...
        if (!empty($transction_status_list_array)) {
            foreach ($transction_status_list_array as $transction_status) {
                if($transction_status['id'] == TRANSACTION_STATUS_ID_PENDING || $transction_status['id'] == TRANSACTION_STATUS_ID_PROCESSED|| $transction_status['id'] == TRANSACTION_STATUS_ID_FAILED)
                {
                    $transction_status['selected'] = false;
                    $transction_status_list[] = $transction_status;
                }                
