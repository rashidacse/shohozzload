...
class Cost_profit_model extends Ion_auth_model {
    ...
    public function get_user_service_profits($user_id, $service_id_list = array()) {
        ...
        return $this->db->select($this->tables['user_profits'] . '.service_id, sum(amount) as total_used_amount, sum(commission) as total_profit,' . $this->tables['services'] . '.title')
    ...
    public function get_profit_history($status_id_list = array(), $service_id_list = array(), $start_time, $end_time, $limit, $offset) {
        ...
        return $this->db->select($this->tables['user_profits'] . '.*,' .$this->tables['user_profits'] . '.id as p_id,' . $this->tables['users'] . '.id as user_id,' . $this->tables['users'] . '.first_name,' . $this->tables['users'] . '.last_name,' . $this->tables['users'] . '.username,' . $this->tables['services'] . '.title as service_title')
                        ->from($this->tables['user_profits'])
     ...
     */
    public function get_report_detail_history($start_time, $end_time, $user_id_list = array()) {
        ...
        return $this->db->select($this->tables['user_transactions'].".status_id,".$this->tables['user_transactions'].".service_id, COUNT(status_id) as total_statuses")
