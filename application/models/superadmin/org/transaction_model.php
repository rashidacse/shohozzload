...
class Transaction_model extends Ion_auth_model {
    ...
    public function get_transaction_list($user_id_list = array(), $service_id_list = array(), $status_id_list = array(), $process_id_list = array(), $cell_no = '', $from_unix_time = 0, $to_unix_time = 0, $offset = 0, $limit = 0) {
        ...
        return $this->db->select($this->tables['user_transactions'] . '.*,' .$this->tables['user_transactions'] . '.id as t_id,' . $this->tables['user_transaction_statuses'] . '.title as status,' . $this->tables['services'] . '.title as service_title,' . $this->tables['service_types'] . '.title as process_type,' . $this->tables['operator_types'] . '.title as type,' . $this->tables['users'] . '.username')
                        ->from($this->tables['user_transactions'])
