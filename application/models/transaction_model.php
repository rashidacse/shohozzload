...
//        }
     ...
     * @param $last_balance, current available balance of the user who is executing the transaction
     * @author nazmul hasan on 9th May 2016
     */

    public function add_transactions($transction_list, $user_profit_list, $last_balance) {
        ...
        foreach ($transction_list as $transaction_info) {
            ...
            $payment_info = array(
                ...
                'status_id' => TRANSACTION_STATUS_ID_PENDING,
                'balance_in' => 0,
                'balance_out' => $transaction_info['cost'],
                'description' => 'Use Service',
                'type_id' => PAYMENT_TYPE_ID_USE_SERVICE,
                'created_on' => $current_time,
                'modified_on' => $current_time
            );
            $last_balance = (double)($last_balance - $transaction_info['cost']);
            if ($transaction_info['process_type_id'] == TRANSACTION_PROCESS_TYPE_ID_AUTO) {
                ...
                if ($service_id == SERVICE_TYPE_ID_BKASH_CASHIN) {
                    ...
                    $transaction_info_for_webservice['APIKey'] = API_KEY_BKASH_CASHOUT;
                ...
                foreach ($user_profit_list as $user_profit_info) {
                    if ($user_profit_info['mapping_id'] == $transaction_info['mapping_id'] && $user_profit_info['user_id'] == $transaction_info['user_id'] && $user_profit_info['commission'] > 0) {
                        $last_balance = (double)($last_balance + $user_profit_info['commission']);
                    }
                }
                $transaction_info['last_balance'] = $last_balance;
                ...
                foreach ($user_profit_list as $user_profit_info) {
                    //if there is commission then it will be added at profit and payment tables
                    if ($user_profit_info['mapping_id'] == $transaction_info['mapping_id'] && $user_profit_info['commission'] > 0) {
                        $user_profit_info['transaction_id'] = $trx_id;
                        ...
                        //adding payment info based on commission of transaction
                        $payment_info_commission = array(
                            'user_id' => $user_profit_info['user_id'],
                            'reference_id' => $user_profit_info['reference_id'],
                            'status_id' => TRANSACTION_STATUS_ID_PENDING,
                            'balance_in' => $user_profit_info['commission'],
                            'balance_out' => 0,
                            'description' => 'Commission',
                            'type_id' => PAYMENT_TYPE_ID_COMMISSION,
                            'created_on' => $current_time,
                            'modified_on' => $current_time,
                            'transaction_id' => $trx_id
                        );
                        $payment_list_manual[] = $this->_filter_data($this->tables['user_payments'], $payment_info_commission);
                    }
                    if ($user_profit_info['mapping_id'] == $transaction_info['mapping_id'] && $user_profit_info['user_id'] == $transaction_info['user_id'] && $user_profit_info['commission'] > 0) {
                        $last_balance = (double)($last_balance + $user_profit_info['commission']);
                    }
                    ...
                    if (property_exists($result_event, "result") != FALSE) {
                        ...
                        if (empty($mapping_info_list)) {
                            ...
                            foreach ($mapping_info_list as $mapping_info) {
                                ...
                                foreach ($user_profit_list as $user_profit_info) {
                                    //if there is commission then it will be added at profit and payment tables
                                    if ($user_profit_info['mapping_id'] == $mapping_info->referenceId && $user_profit_info['commission'] > 0) {
                                        ...
                                        $payment_info_commission = array(                                                
                                            ...
                                            'created_on' => $current_time,
                                            'modified_on' => $current_time,
                                            'transaction_id' => $mapping_info->transactionId
                                        );
                                        $payment_list_auto[] = $this->_filter_data($this->tables['user_payments'], $payment_info_commission);
                                    }
                                }
                            }
                            $this->db->trans_begin();
    ...
    //Don't remove this method, it may be used in future to allow update transaction
    ...
    public function get_user_transaction_list($user_id_list = array(), $service_id_list = array(), $status_id_list = array(), $cell_no = '', $from_date = 0, $to_date = 0, $limit = 0, $offset = 0) {
        ...
        return $this->db->select($this->tables['user_transactions'] . '.*,' .$this->tables['user_transactions'] . '.id as t_id,' . $this->tables['user_transaction_statuses'] . '.title as status,' . $this->tables['services'] . '.title as service_title,' . $this->tables['operator_types'] . '.title as type,' . $this->tables['users'] . '.username')
                        ->from($this->tables['user_transactions'])
                        ...
                        ->join($this->tables['operator_types'], $this->tables['operator_types'] . '.id=' . $this->tables['user_transactions'] . '.operator_type_id')
